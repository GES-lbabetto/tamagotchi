...
with tmp(mode="w+") as topo_tmp, tmp(mode="w+") as xyz_tmp:
    if selection.mol2 and selection.xyz:
        topo_tmp.write(
        ...
        def create_u():
            ...
            u = mda.Universe(
                topo_tmp.name, xyz_tmp.name, format="XYZ", topology_format="MOL2"
            )
        ...
        with tab1:
            ...
            with tab1_col2:
                ...
                if "fig_rdf" not in ss or st.button(f"🧹 Clear {atom1}-{atom2} RDF"):
                    ...
                    if show_water:
                        exp_path = f"{os.path.dirname(__file__)}/../data/RDF_water_{atom1}{atom2}.csv"
